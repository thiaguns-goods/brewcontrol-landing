...
if (format === 'poster') {
  await renderStill({composition, serveUrl, output: `${base}-poster.webp`, frame: 0, imageFormat: 'webp', inputProps});
} else {
  await renderMedia({composition, serveUrl, codec: format === 'webm' ? 'vp8' : 'h264', outputLocation: `${base}.${format}`, audioCodec: null, inputProps, frameRange: [0, durationInFrames - 1]});
}
};
