...
RAW and outputs default to `public/raw/` and `output/`, but can live entirely outside the repository: `npm run render -- --demo production --format webm --raw-dir C:\\captures --output-dir C:\\renders`. Each catalog source is a filename such as `production.webm`, resolved relative to `--raw-dir`. Use `--trim-start-frame` and `--trim-end-frame` for an explicit local proof or approved trim. Optional `--poster-frame` sets the frame index relative to the composition (0-indexed) for poster generation. Optional `--crf` (or `--webm-crf` / `--mp4-crf`) tunes compression quality generically. The bundle uses Remotion's temporary directory, outputs never overwrite RAW, and no cache binary is written to the worktree. Defaults are 1440x810, 30 fps (or demo-configured fps, e.g. 25 fps for assets), muted, neutral 1:1 presentation, no effects or transitions. WebM uses VP8 and MP4 H.264; prioritize legibility (targets <=1.5 MB / <=2 MB).
